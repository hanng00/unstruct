...
export interface IFileRepository {
  ...
  listFiles(userId: string): Promise<{ success: boolean; files?: FileReference[]; error?: string }>;
}
...
export class DDBFileRepository implements IFileRepository {
  ...
  async listFiles(userId: string): Promise<{ success: boolean; files?: FileReference[]; error?: string }> {
    try {
      ...
      const files = result.Items?.map(item => item.fileData as FileReference) || [];
